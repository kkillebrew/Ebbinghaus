...
subjlist = {
    ...
    'KEN'
    'BKN'};
% 'DNS' - Chris isn't sure who this was, may be a mis-labeled Emma, who isn't around anymore



% save command window output to textfile for quick access to stats
diary_file = './outputs/lc_output.txt';
...
for subjcell = subjlist'
    ...
    for metriccell = which_metrics
        ...
        % for revamp, we only care about condition 5
        eval(sprintf('g.(metric).raw(subj_i,1) = %s(:,5);',metric));
    end
...
box off
%saveas(gcf,'./figures/lc_group.fig'); % save figure to a file
print('-dpdf','./figures/lc_group.pdf')
